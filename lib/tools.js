...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Find investors from NUVC's 3,700+ investor database who are the best fit for a startup. " +
      "Uses AI semantic matching (embeddings) to compare the startup description against each investor's " +
      "thesis, focus areas, stage preference, and track record. Returns ranked matches with thesis " +
      "alignment reasoning, check size ranges, and investor quality signals. " +
...
async function handleMatchInvestors(args) {
  ...
  const lines = [
    ...
    `**${matches.length} investors** matched from ${data.pool_size || "3,700+"} database | ` +
